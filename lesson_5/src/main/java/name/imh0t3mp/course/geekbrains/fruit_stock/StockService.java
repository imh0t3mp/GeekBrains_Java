...
public class StockService {
    ...
    public void shiftBoxes(Box b1, Box b2)
            throws WrongTypeOfFruitInBox, WrongBoxCapacity, NoFreeSpaceInBox, BoxIsEmpty {
...
//        }
     ...
     */
    public float getNetto(Box box) throws BoxNotFound {
        if (!stock.hasBox(box))
     ...
     */
    public float getNetto(String boxId) throws BoxNotFound {
        if (!stock.hasBox(boxId))
            throw new BoxNotFound("На складе не найден ящик #" + boxId);
        return stock.getBox(boxId).getNetto();
    }
}
